document.addEventListener("DOMContentLoaded", () => {
   ...
   let parallax = document.querySelector("._parallax");
   let parallaxMob = document.querySelector("._parallax__mob");

   let tl = gsap.timeline();
   let tlMobile = gsap.timeline();

   tlMobile.fromTo(parallaxMob, {x: 0}, {x: -100});

   if (window.innerWidth >= 768) {
      tl.fromTo(parallax, {y: 0}, {y: -250});
...
document.addEventListener("DOMContentLoaded", () => {
   ...
   let videoVisibleTwo = function (targetTwo) {
      ...
      if (targetTwoPosition.bottom > windowPosition.top && targetTwoPosition.top < windowPosition.bottom) {
         while (jStop < 1) {
